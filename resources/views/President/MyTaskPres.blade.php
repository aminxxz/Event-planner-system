...
<section class="content">
    <div class="container-fluid">
      <div class="row">
        <div class="col-12">
          @foreach ($eventListRolePres as $eventTask)
          ...
          <div class="card">
            ...
            <div class="card-body">
              ...
              <table id="TaskEventList" class="table table-bordered table-hover">
                ...
                <tbody>
                  @foreach ($eventTaskListPres as $task)
                <tr>
                  <td>{{$task->task_name}}</td>
                  <td>{{$task->task_status}}</td>
                  <td><a href="../roleViewTaskPres/{{$task->task_id}}"><button>View Task</button></a>
                    @if($task->task_status == 'On-going')
                    <form action="../taskCompletePres/{{$task->task_id}}" method="POST">
                      @csrf
                    ...
                    <form action="../taskDeletePres/{{$task->task_id}}" method="POST">
                      @csrf
              ...
              <br>
              @if ($eventTask->event_status != 'Complete')
              <a href="../addTaskPres/{{$eventTask->role_id}}" ><button class="btn btn-primary">Add Tasks</button></a>
            @else
            <a href="../viewPMRolePres/{{$eventTask->event_id}}" ><button class="btn btn-primary">Post Mortem</button></a>

            <button class="btn btn-primary" data-toggle="modal" data-target="#add-post-mortem">Add Post Mortem</button>
            @endif
            </div>
            <!-- /.card-body -->
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form action="myTaskPres2/{{$eventTask->role_id}}" method="POST">
                            @csrf
